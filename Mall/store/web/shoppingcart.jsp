...
<div id="templatemo_wrapper">
	...
	<div id="templatemo_header">
    	<div id="site_title"><h1><a href="http://localhost：8080/${pageContext.request.contextPath }">LOL在线皮肤商城</a></h1></div>
        <div id="header_right">
        	<p>
	        <c:if test="${!empty user }">
	        	<a href="${pageContext.request.contextPath }/user/personal.jsp">我的个人中心</a> |
	        </c:if>
	        <a href="${pageContext.request.contextPath }/shoppingcart.jsp&uid=${user.uid}">购物车</a> |
	        <c:if test="${user == null }">
	        ...
	        <p>
		        <c:if test="${!empty user }">
		        	<a href="${pageContext.request.contextPath }/frontEndOrderServlet?op=myoid&uid=${user.uid}">我的订单</a> |
		        </c:if>
	        </p>
		</div>
        	...
        	<form action="">
	        	<table width="680px" cellspacing="0" cellpadding="5">
	                   	  <tr bgcolor="#ddd">
	                        	...
	                        	<th width="180" align="left">皮肤名字 </th>
	                      ...
	                      <c:if test="${!empty user }">
		                      <c:forEach items="${cart.cartItemList}" var="item">
									<tr>
			                        	<td><img src="${item.product.imgurl }" style="width: 90px;height: 150px" alt="" /></td>
			                        	<td>${item.product.pname }</td> 
			                            <td align="center"><font style="width: 20px; text-align: right" />${item.num }</font> </td>
			                            <td align="right">${item.product.estoreprice} </td>
			                            <td align="right">${item.product.estoreprice*item.num } </td>
			                            <td align="center"> 
			                            	<a href="${pageContext.request.contextPath }/CartServlet?op=deleteItem&uid=${user.uid}&itemid=${item.product.pid}">
                    ...
                    <c:if test="${!empty cart.cartItemList}"><p><a href="${pageContext.request.contextPath }/placeOrder.jsp">立即购买</a></p></c:if>
