...
<div id="templatemo_wrapper">
    ...
    <div id="templatemo_main">
        ...
        <div id="content" class="float_r">
        	 ...
        	 <h3>确认订单</h3>
	<form action="${pageContext.request.contextPath }/frontEndOrderServlet" method="post">
	...
	<div>
		<table style="width: 700px;border-style:1px solid " border="1">
			<tr>
				<td>收件人:</td>
				<td><input type="text" name="recipient" required="required"/></td>
			</tr>
			<tr>
				<td>电话:</td>
		...
		<table style="width: 700px;border-style:1px solid " border="1">
			...
			<c:forEach items="${cart.cartItemList}" var="item">
				<tr id="tr${item.product.pid}">
					...
					<td><img src="${item.product.imgurl}" width="60" height="100" /></td>
					<td>${item.product.pname}</td>
					<td>${item.num}</td>
					<td>${item.product.estoreprice}</td>
					<td id="price">${item.num*item.product.estoreprice}</td>
				</tr>
				<c:set var="sum" value="${sum+item.num*item.product.estoreprice}"> </c:set>
			</c:forEach>
		</table>
	</div>
