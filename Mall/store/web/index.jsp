...
<div id="templatemo_wrapper">
	...
	<div id="templatemo_header">
    	<div id="site_title"><h1><a href="http://localhost:8080/${pageContext.request.contextPath }"></a></h1></div>
        <div id="header_right">
        	<p>
        	<c:if test="${!empty user }">
		        <a href="${pageContext.request.contextPath }/user/personal.jsp">我的个人中心</a> |
				<a href="${pageContext.request.contextPath }/CartServlet?op=findCart&uid=${user.uid}">购物车</a> |
        	</c:if>
	        ...
	        <c:if test="${!empty user }">
	        	${user.nickname}
	        	<a href="${pageContext.request.contextPath }/frontEndUserServlet?op=logout">退出</a></p>
	        </c:if>
		</div>
    ...
    <div id="templatemo_menubar">
        ...
        <div id="templatemo_search">
            <form action="${pageContext.request.contextPath }/frontEndProductServlet" method="get">
                		...
                		<c:forEach items="${categories }" var="category" varStatus="vs">
                			<c:if test="${vs.index !=0}">
                				<c:if test="${vs.index != fn:length(categories)-1 }">
                					<li>
                						<a href="${pageContext.request.contextPath }/frontEndProductServlet?op=findProductByCid&cid=${category.cid}">${category.cname}</a>
                					</li>
                				</c:if>
                			</c:if>
                			<c:if test="${vs.index==0 }">
        ...
        <div id="content" class="float_r">
        	<div id="slider-wrapper">
                <div id="slider" class="nivoSlider">
                	<c:forEach items="${productTop}" var="product">
						<a href="${pageContext.request.contextPath}/frontEndProductServlet?op=findProductByCid&cid=${product.cid}">
        	...
        	<c:forEach items="${hotProducts }" var="product" varStatus="vs">
        		<div class="${vs.index % 3 != 2?'product_box':'product_box no_margin_right' }">
		            <h3>${product.pname }</h3>
	            	<a href="${pageContext.request.contextPath }/frontEndProductServlet?op=findProductById&pid=${product.pid}">
	            		<img src="${product.imgurl }" width="180" height="300" alt="" title="${product.pname }"/>
	            	</a>
	                ...
	                <a href="${pageContext.request.contextPath }/frontEndProductServlet?op=findProductById&pid=${product.pid}" class="detail"></a>
            	</div>
	...
	<script type="text/javascript">
		...
		function addCart(pid,uid){
			window.location.href="${pageContext.request.contextPath}/CartServlet?op=addCart&pid=" + pid + "&uid=" + uid + "&num=1";
		}
	</script>
</body>
</html>